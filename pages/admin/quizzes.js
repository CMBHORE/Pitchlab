...
export default function AdminQuizzes() {
  ...
  // Reads a File into base64 and sends it to our own server, which
  // uploads it into the shared Google Drive folder and hands back a
  // usable link — this replaces the old direct-to-Supabase upload.
  const uploadFileToDrive = async (file, quizId) => {
    ...
    const filename = `${quizId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${file.name}`;
  ...
  const uploadReferenceImages = async (quizId, files) => {
    ...
    for (const file of files) {
      try {
        const url = await uploadFileToDrive(file, quizId);
        uploaded.push(url);
      } catch (e) {
  ...
  const removeReferenceImage = (quizId, url) => {
    const draft = getQForm(quizId);
    setQ(quizId, { reference_images: (draft.reference_images || []).filter((u) => u !== url) });
  };
  ...
  const uploadQuestionMedia = async (quizId, file) => {
    ...
    try {
      const url = await uploadFileToDrive(file, quizId);
      setQ(quizId, { media_url: url, media_type: isVideo ? "video" : "image" });
    } catch (e) {
      setMsg("⚠ Media upload failed: " + e.message + " — check that GOOGLE_SERVICE_ACCOUNT_KEY and GOOGLE_DRIVE_FOLDER_ID are set correctly in Vercel.");
    }
  };
